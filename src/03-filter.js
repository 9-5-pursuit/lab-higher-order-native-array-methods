...
function getSongsBySaib(songs) {
  let result = songs.filter((element) => element.artist === "Saib");
  return result;
}
...
function getSongsOverThreeMinutes(songs) {
  let result = songs.filter((element) => element.runtimeInSeconds > 180);
  return result;
}
...
function getTitleTracks(songs) {
  let result = songs.filter((element) => element.title === element.album);
  return result;
}
