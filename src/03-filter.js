 ...
 */
//.filter() transverse the array and returns a new array
//create
function getSongsBySaib(songs) {
  let songsByArtist = songs.filter((element) => {
...
function getSongsOverThreeMinutes(songs) {
  let threeMinRuntime = songs.filter((element) => {
    if (element.runtimeInSeconds > 180) {
      return element;
    }
  });
  return threeMinRuntime;
}
...
function getTitleTracks(songs) {
  let songAndAlbum = songs.filter((element) => {
