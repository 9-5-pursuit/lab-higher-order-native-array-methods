...
function getSongsBySaib(songs) {
  let saibSongs = songs
    .filter((element) => element.artist === "Saib")
    .map((el) => el);
  return saibSongs;
}
...
function getSongsOverThreeMinutes(songs) {
  let threeMinSongs = songs
    .filter((element) => element.runtimeInSeconds >= 180)
    .map((el) => el);
...
function getTitleTracks(songs) {
  let songDetails = songs
    .filter((element) => element.title === element.album)
    .map((el) => el);
  return songDetails;
}
