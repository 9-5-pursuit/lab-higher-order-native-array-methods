...
function getSongTitles(songs) {
  return songs.map((song) => song.title); //map method is used to loop over each item in the array then return new array with title of songs.
}
...
function getSongDetails(songs) {
  return songs.map((song) => `${song.title} by ${song.artist}`); //arrow function is used to take in song. String contains title and artist of each song.
}
