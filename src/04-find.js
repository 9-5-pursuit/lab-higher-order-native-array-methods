...
function findPinkElephantsByTimestreet(songs) {
  let pinkElephants = songs.find((element) => element.title === "Pink Elephants")
...
function findFirstSongUnderThreeMinutes(songs) {
  let threeMinutes = songs.find((element) => element.runtimeInSeconds < 180)
  return threeMinutes
}
...
function findFirstTitleTrack(songs) {
  let songAlbum = songs.find((element) => element.title === element.album)
  return songAlbum
}
