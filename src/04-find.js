...
function findPinkElephantsByTimestreet(songs) {
  ...
  let result = songs.find(song => song.title === "Pink Elephants")
  return result;  
}
...
function findFirstSongUnderThreeMinutes(songs) {
  let result = songs.find(song => song.runtimeInSeconds < 180)
  return result;
}
...
function findFirstTitleTrack(songs) {
  let result = songs.find(song => song.title === song.album)
  return result;
}
