...
function findPinkElephantsByTimestreet(songs) {
  let search = songs.find(song => song.title === "Pink Elephants")
  return search
}
...
function findFirstSongUnderThreeMinutes(songs) { 
  let search = songs.find(song => song.runtimeInSeconds < 180)
return search
}
...
function findFirstTitleTrack(songs) {
  let search = songs.find(song => song.album === song.title)
  return search}
