...
function printAllSongTitles(songs) {
  songs.forEach((song) => {
    //the forEach() method loops over each object in the array
...
function printSongDetails(songs) {
  songs.forEach((song) => {
    console.log(`${song.title} by ${song.artist}`); //title and artist of the song are logged using string interpolation.
  });
}
...
function printSongTitlesOverThreeMinutes(songs) {
  songs.forEach((song) => {
    ...
    if (song.runtimeInSeconds > 180) {
      //if statement is used to check the runtime of the song.
      console.log(song.title); //logs title of song if runtime is over 3 mins (180 seconds)
    }
  });
}
