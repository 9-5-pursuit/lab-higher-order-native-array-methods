...
function printAllSongTitles(songs) {
  ...
  songs.forEach(item => console.log(item.title))
}
...
function printSongDetails(songs) {
  ...
  songs.forEach(item => console.log(item.title + ' by ' + item.artist))
}
...
function printSongTitlesOverThreeMinutes(songs) {
  ...
  songs.forEach(item => {if (item.runtimeInSeconds>180) console.log(item.title)})
}
