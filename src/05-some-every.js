...
function allSongsAreOverTwoMinutes(songs) {
  let result = songs.every((element) => element.runtimeInSeconds < 180);
  return result;
}
...
function anySongIsOverFourMinutes(songs) {
  let result = songs.some((element) => element.runtimeInSeconds > 240);
  return result;
}
...
function anySongIsByPeanut(songs) {
  let result = songs.some((element) => element.artist === "Peanut");
  return result;
}
