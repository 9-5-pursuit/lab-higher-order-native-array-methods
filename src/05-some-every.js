...
function allSongsAreOverTwoMinutes(songs) {
  const result = songs.every(song => song.runtimeInSeconds > 120);
  return result
}
...
function anySongIsOverFourMinutes(songs) {  
  const result = songs.some(song => song.runtimeInSeconds > 240);
  return result
}
...
function anySongIsByPeanut(songs) {
  const result = songs.some(song => song.artist === "Peanut");
  return result
