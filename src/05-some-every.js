...
function allSongsAreOverTwoMinutes(songs) {
  let overTwoMins = songs.every((element) => element > 120);
...
function anySongIsOverFourMinutes(songs) {
  let overFour = songs.some((element) => element.runTimeInSeconds > 240);
...
function anySongIsByPeanut(songs) {
  let isArtist = songs.some((element) => element.artist === "Peanut");
