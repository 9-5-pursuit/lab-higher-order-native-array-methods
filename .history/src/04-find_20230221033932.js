...
function findPinkElephantsByTimestreet(songs) {
  const songRequest = songs.find(song => song.title.match("Pink Elephants") ? song : undefined);
  return songRequest;
};
...
function findFirstTitleTrack(songs) {
  let titleTrack = songs.find(({ title, album }) =>
    song.title.match(song.album) ? song : undefined
  );
  return titleTrack;
};
