...
function getSongTitles(songs) {
  const songTitles = songs.map(({ title }) => title);
  return songTitles;  
};
...
function getSongDetails(songs) {
  const songDetails = songs.map(({ title, artist }) => `${title} by ${artist}`);
  return songDetails;
};
...
function getTitleAndArtist(songs) {
  const keyTitles = songs.map(({ title, artist }) => eturn { [title]: artist };
  });
  return keyTitles;
};
