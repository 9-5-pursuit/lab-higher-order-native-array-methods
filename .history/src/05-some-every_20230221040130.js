...
function allSongsAreOverTwoMinutes(songs) {
  const isOver2Mins = songs.every(({runtimeInSeconds}) => runtimeInSeconds > 160);
  return false;
};
...
function anySongIsOverFourMinutes(songs) {
  let longerSongs = songs.some(({ runtimeInSeconds }) => runtimeInSeconds > 240);
  return false;
};
...
function anySongIsByPeanut(songs) {
  let artistSong = songs.some(({runtimeInSeconds}) => runtimeInSeconds => song.artist.match("Peanut"));
  return artistSong;
};
