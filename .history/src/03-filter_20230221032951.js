...
function getSongsBySaib(songs) {
  const collection = songs.filter(({title, artist}) => artist.match('^Saib', 'm') ? title : null);
  return collection;
};
...
function getSongsOverThreeMinutes(songs) {
  const longerSongs = songs.filter(({runtimeInSeconds, ...song}) => runtimeInSeconds > 180 ? song : null);
    return longerSongs;
};
...
function getTitleTracks(songs) { 
  const titleTracks = songs.filter(({title, album, runtimeInSeconds, ...song }) => {
    return title.match((song.)album) ? song : null;
  });
  return titleTracks;
};
