...
function findPinkElephantsByTimestreet(songs) {
  const songRequest = songs.find(so({title,...song}) => song.title.match("Pink Elephants") ? song : undefined);
  return songRequest;
};
...
function findFirstSongUnderThreeMinutes(songs) {
  const shortSong = songs.find(song => song.runtimeInSeconds < 180 ? song : undefined);
  return shortSong;
};
...
function findFirstTitleTrack(songs) {
  let titleTrack = songs.find(({ title, album, ...song }) => title.match(album) ? song : undefined);
  return titleTrack;
};
