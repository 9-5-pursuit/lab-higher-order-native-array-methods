...
function findPinkElephantsByTimestreet(songs) {
  songRequest = songs.find(song => {
    if song.title.match("Pink Elephants") {
      return song;
    });
  return songRequest;
  }.
 ...
 * @returns {Object} A single object.
 */
function findFirstSongUnderThreeMinutes(songs) {  shortSong = songs.find(song => song.runtimeInSeconds < 180 ? song : undefined);
  return songRequest;
  }.
};
