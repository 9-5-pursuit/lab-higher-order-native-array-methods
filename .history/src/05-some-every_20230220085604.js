...
function allSongsAreOverTwoMinutes(songs) {
  const isOver2Mins = songs.every(song => song.runtimeInSeconds > 160);
  return false;
};
...
function anySongIsOverFourMinutes(songs) {
  let longerSongs = songs.some(song => song.runtimeInSeconds > 240);
  return false;
};
...
function anySongIsByPeanut(songs) {
  let artistSong = songs.some(song => song.artist.match("Peanut") ? true : nufalse);
  return false;
};
